...
반면 음수인 경우 절댓값만큼 크기를 할당하고 왼쪽에 끝에 붙여서 문자열을 출력한다.<br/>
...
서식 문자열은 다음과 같은게 있다.<br/>


|서식 지정자|대상서식|기능|
|:---:|:---:|:---:|
|D|10진수|입력된 수를 10진수로 서식화 한다.|
|X|16진수|입력된 수를 16진수로 서식화 한다.|
|N|10진수|입력된 수를 콤마(,)로 구분해 출력한다.|
|F|고정소수점|입력된 수를 고정소수점 형식으로 서식화한다.|
|E|지수|입력된 수를 지수 표기로 서식화 한다.|


이외에도 B, G등 다양하게 존재한다.<br/>
자세한건 msdn의 표준 숫자 형식 문자열 항목을 확인하면 된다.<br/>
링크 : https://learn.microsoft.com/ko-kr/dotnet/standard/base-types/standard-numeric-format-strings<br/>


DateTime 클래스를 이용해 날짜와 시간도 서식문자로 표현할 수 있다.<br/>
해당 부분은 msdn의 표준 날짜 및 시간 형식 문자열을 확인하면 된다.<br/>
링크 : https://learn.microsoft.com/ko-kr/dotnet/standard/base-types/standard-date-and-time-format-strings<br/>


## 문자열 보간
Format() 메소드와 더불어 서식화된 문자열을 편리하게 다룰 수 있는 또다른 방법이다.<br/>
C# 6.0에서 새로 도입된 기능으로, 한결 더 편리하게 문자열의 양식을 맞출 수 있다.<br/>

문자열 보간은 문자열 앞에 \$를 써서 나타낸다.<br/>
...
의 형태로 쓴다.<br/>
  - 보간식은 리터럴 상수나 변수를 넣는 위치다. 
  - 길이는 Format의 맞춤과 같고 서식 문자열은 Format의 서식문자열과 같다.