...
## 분할 클래스
분할 클래스(Partial Class)란 여러 번에 나눠서 구현하는 클래스를 말한다.<br/>
partial 키워드를 앞에 붙여 구현할 수 있다.<br/>
클래스의 구현이 길어질 경우 여러 파일에 나눠서 구현할 수 있게 함으로써 소스코드의 편의를 제공해준다.<br/>


## 확장 메소드
확장 메소드(Extension Method)는 기존 클래스의 기능을 확장하는 기법이다.<br/>
확장 메소드는 기존 클래스의 기능을 확장하기에 상속과는 다르다.<br/>
확장 메소드를 선언하는 방법은 메소드를 선언하되 static 한정자로 수식해야 한다<br/>
그리고 첫 번째 매개변수는 반드시 this 키워드와 확장하고자 하는 클래스(형식)의 인스턴스여야 한다.<br/>
그 뒤에 따라오는 매개변수 목록은 확장 메소드를 호출할 때 입력되는 매개변수이다.<br/>
선언하는 메소드 역시 static이어야 하고 상위 클래스도 static이어야 한다.<br/>


## 구조체
C#의 복합 데이터 형식에는 클래스 말고도 구조체(Structure)가 있다.<br/>
구조체는 필드와 메소드를 가질 수 있어 클래스와 비슷하다.<br/>
구조체의 선언은 struct 키워드를 이용해 클래스처럼 선언하면 된다.<br/>
...
클래스는 실세계의 객체를 추상화하려는 데 그 존재의 이유가 있지만, 구조체는 데이터를 담기 위한 자료구조로 사용된다.<br/>
그래서 굳이 은닉성을 비롯한 객체지향의 원칙을 구조체에 강하게 적용하지 않는 편이며, 편의를 위해 필드를 public으로 선언해서 사용한다.<br/>

구조체는 값 형식이므로 구조체의 인스턴스는 스택에 할당되고 인스턴스가 선언된 블록이 끝나는 지점에 메모리에서 사라진다.<br/>
그래서 인스턴스의 사용이 끝나면 즉시 메모리에서 제거된다는 점과 가비지 콜렉터를 신경쓰지 않아도 된다는 점에 성능상 이점이 있다.<br/>

구조체는 new 연산자를 사용하지 않아도 인스턴스를 만들 수 있고 CLR이 구조체의 각 필드를 기본값으로 초기화 해준다.<br/>

객체의 상태 변화를 허용하는 객체를 변경가능(Mutable) 객체라고 하며, 상태의 변화를 허용하지 않는 객체를 변경불가능(Immutable) 객체라고 한다.<br/>
변경불가능 객체는 멀티 쓰레드 간에 동기화(Synchronization)를 할 필요가 없기 때문에 프로그램 성능 향상이 가능하고,<br/>
버그로 인한 데이터의 변경을 막을 수 있다.<br/>

구조체는 readonly 키워드를 이용해 모든 필드와 프로퍼티의 값을 수정할 수 없는, 변경불가능 구조체로 선언할 수 있다.<br/>
클래스는 불가능하고, 구조체의 생성자에서 초기 값을 할당하면 된다.<br/>


	readonly struct ImmutableStructure
	{

	    public int val;

	    public ImmutableStructure(int _val)
	    {
...
그리고 구조체의 메소드에 readonly 키워드를 사용해 메소드 내부에서 구조체 필드 값을 변경못하게 막게할 수 있다.<br/>


## 튜플
튜플(Tuple)도 여러 필드를 담을 수 있는 구조체이다.<br/>
튜플은 형식 이름이 없다. 그래서 튜플은 응용 프로그램 전체에서 사용할 형식을 선언할 때가 아닌,<br/>
즉석에서 사용할 복합 데이터 형식을 선언할 때 적합하다.<br/>
튜플은 구조체이므로 값 형식이고 아래처럼 구현한다.<br/>


	var tuple = ("ABC", 123, 3.1415);


튜플은 위처럼 필드의 이름을 지정하지 않는 경우 명명되지 않은 튜플(Unnamed Tuple)이라 한다.<br/>
튜플은 System.ValueTuple 구조체를 기반으로 만들어진다.<br/>
튜플은 필드의 아래처럼 이름을 지정해 명명된 튜플(Named Tuple)로 가독성 좋게할 수 있다.<br/>


	var tuple1 = (String: "ABC", Int: 123, Double: 3.1415);
	(string String, int Int, double Double) tuple2 = ("ABC", 123, 3.1415);


튜플은 분해자(Deconstructor)를 구현하고 있기 때문에 분해도 가능하다


	var tuple = (Name: "ABC", Age: 22);
	var (name1, age) = tuple;				// 분해
	var (name2, _) = tuple;					// Age 필드는 무시


분해한 결과를 siwtch 문이나 switch 식의 분기 조건에도 활용할 수 있다.<br/>
이를 패턴 매칭(Positional Pattern Matching)이라고 한다.<br/>
식별자나 데이터 형식이 아닌 분해된 요소의 위치에 따라 값이 일치하는지 판단한다.<br/>